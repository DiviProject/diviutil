...
func TestFilterInsert(t *testing.T) {
	...
	err = f.MsgFilterLoad().DiviEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsert DiviDecode failed: %v\n", err)
		return
	}
...
func TestFilterFPRange(t *testing.T) {
	...
	for _, test := range tests {
		...
		err = f.MsgFilterLoad().DiviEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
		if err != nil {
			t.Errorf("DiviDecode unexpected error: %v\n", err)
			continue
		}
		if !bytes.Equal(got.Bytes(), want) {
...
func TestFilterInsertWithTweak(t *testing.T) {
	...
	err = f.MsgFilterLoad().DiviEncode(got, wire.ProtocolVersion, wire.LatestEncoding)
	if err != nil {
		t.Errorf("TestFilterInsertWithTweak DiviDecode failed: %v\n", err)
		return
	}
...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := diviutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(diviutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := diviutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := diviutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
...
func TestFilterInsertP2PubKeyOnly(t *testing.T) {
	...
	block, err := diviutil.NewBlockFromBytes(blockBytes)
	if err != nil {
