...
func (cs *CoinSet) TotalValue() (value diviutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue diviutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue diviutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount diviutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue diviutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue diviutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue diviutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue diviutil.Amount, coins []Coin) (Coins, error) {
...
// diviutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() diviutil.Amount {
	return diviutil.Amount(c.txOut().Value)
}
